...
public:
    ...
    void SetText(const wxString& text)
    {
        ...
        if (!text.empty()) {
            ...
            memDC.SetTextForeground(wxColour(232, 135, 26)); // categorical 3 colortag
    ...
    void Decorate(wxBitmap& bmp)
    {
        ...
        memDc.SetTextForeground(wxColour(250, 250, 250)); // 98% near-white colortag
...
{
    return dark_mode() ? wxColour(71, 226, 111) : wxColour(38, 141, 108); // categorical 6 : categorical 11 colortag
}

const wxColour GUI_App::get_label_default_clr_modified()
{
    return dark_mode() ? wxColour(232, 135, 26) : wxColour(203, 111, 25); // categorical 3 : categorical 10 colortag
}
...
#ifdef _WIN32
    m_color_label_default           = is_dark_mode ? wxColour(250, 250, 250): wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT); // 98% near-white colortag
    m_color_highlight_label_default = is_dark_mode ? wxColour(250, 250, 250): wxSystemSettings::GetColour(/*wxSYS_COLOUR_HIGHLIGHTTEXT*/wxSYS_COLOUR_WINDOWTEXT); // 98% near-white colortag
    m_color_highlight_default       = is_dark_mode ? wxColour(64, 64, 64)   : wxColour(240, 240, 240); // 25.1% mono shade : 94.1% mono tint colortag
    m_color_hovered_btn_label       = is_dark_mode ? wxColour(232, 135, 26) : wxColour(203, 111, 25); // categorical 3 : categorical 10 colortag
    m_color_selected_btn_bg         = is_dark_mode ? wxColour(43, 43, 43)   : wxColour(255, 255, 255); // 16.9% mono shade : 100% white colortag
#else
    m_color_label_default = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
#endif
    m_color_window_default          = is_dark_mode ? wxColour(43, 43, 43)   : wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW); // 16.9% mono-shade colortag
}
...
void GUI_App::UpdateDVCDarkUI(wxDataViewCtrl* dvc, bool highlited/* = false*/)
{
    ...
    if (!dark_mode())
        dvc->SetBackgroundColour(wxColour(250, 250, 250)); // 98% near-white colortag
#endif //_MSW_DARK_MODE
...
void GUI_App::set_auto_toolbar_icon_scale(float scale) const
{
    ...
    long int_val = std::min(int(std::lround(scale / icon_sc * 200)), 200);
...
void GUI_App::update_ui_from_settings()
{
    ...
    if (m_force_colors_update) {
        ...
        wxGetApp().plater()->canvas3D()->get_gizmos_manager().force_dirty_gizmos();
#ifdef _MSW_DARK_MODE
