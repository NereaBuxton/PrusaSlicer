    ...
    for (const std::string& filename : filenames) {
        ...
        for (int i = 1; i < sprite_n_pixels; ++i) {
            ...
            if (sprite_white_only_data.data()[offset] != 0)
                ::memset((void*)&sprite_white_only_data.data()[offset], wxGetApp().dark_mode() ? 43 : 240, 3);
        }
            ...
            if (state.second) {
                ...
                for (unsigned int r = 1; r <= sprite_size_px; ++r) {
                    ...
                    for (unsigned int c = 1; c <= sprite_size_px; ++c) {
                        ...
                        output_data.data()[offset + 3] = (unsigned char)((wxGetApp().dark_mode() ? 64 : 255) * (1.0f - alpha) + output_data.data()[offset + 3] * alpha);
                    }
                }
            }
