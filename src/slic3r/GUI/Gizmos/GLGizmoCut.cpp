...
const std::array<float, 4> GLGizmoCut::GrabberColor = { 0.910, 0.529, 0.102, 1.0 }; // categorical 2
...
void GLGizmoCut::on_render()
{
    ...
    ::glBegin(GL_QUADS);
    ::glColor4f(0.942f, 0.942f, 0.942f, 0.502f); // 94% white 50% alpha
    ...
    glsafe(::glColor3f(0.875, 0.749, 0.098)); // categorical 1
    ::glBegin(GL_LINES);
    ...
    if (0.0 < m_cut_z && m_cut_z < m_max_z) {
        if (m_cut_contours.cut_z != m_cut_z || m_cut_contours.object_id != model_object->id() || m_cut_contours.instance_idx != instance_idx) {
            ...
            if (!polys.empty()) {
                ...
                m_cut_contours.contours.set_color(-1, { 0.980f, 0.980f, 0.980f, 1.0f }); // 98% white
            }
        }
