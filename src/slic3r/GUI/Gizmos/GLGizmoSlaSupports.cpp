...
void GLGizmoSlaSupports::render_points(const Selection& selection, bool picking) const
{
    ...
    for (size_t i = 0; i < cache_size; ++i) {
        ...
        if (picking)
            ...
            if (size_t(m_hover_id) == i && m_editing_mode) // ignore hover state unless editing mode is active
                ...
                if (m_editing_mode) {
                    if (point_selected)
                        render_color = { 0.910f, 0.529f, 0.102f, 1.0f}; // categorical 2
                    else
                        if (supports_new_island)
                            render_color = { 0.098f, 0.753f, 0.780f, 1.0f }; // categorical 11
                        else
                            render_color = { 0.753f, 0.753f, 0.753f, 1.0f }; // 75% white
                }
                else
                    render_color = { 0.502f, 0.502f, 0.502f, 1.0f }; // 50% neutral
            }
        }
    ...
    if (has_holes && !picking) {
        render_color[0] = 0.753f; // 75% white 75% alpha
