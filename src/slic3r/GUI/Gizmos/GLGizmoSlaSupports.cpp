...
void GLGizmoSlaSupports::render_points(const Selection& selection, bool picking) const
{
    ...
    for (size_t i = 0; i < cache_size; ++i) {
        ...
        if (picking)
            ...
            if (size_t(m_hover_id) == i && m_editing_mode) // ignore hover state unless editing mode is active
                render_color = { 0.153f, 0.502f, 0.922f, 1.0f }; // categorical 7 colortag
            else { // neigher hover nor picking
                bool supports_new_island = m_lock_unique_islands && support_point.is_new_island;
                if (m_editing_mode) {
                    if (point_selected)
                        render_color = { 0.910f, 0.529f, 0.102f, 1.0f}; // categorical 3 colortag
                    else
                        if (supports_new_island)
                            render_color = { 0.098f, 0.753f, 0.780f, 1.0f }; // categorical 1 colortag
                        else
                            render_color = { 0.753f, 0.753f, 0.753f, 1.0f }; // 75.3% mono tint colortag
                }
                else
                    render_color = { 0.502f, 0.502f, 0.502f, 1.0f }; // 50.2% mono tone colortag
            }
        }
    ...
    if (has_holes && !picking) {
        render_color[0] = 0.753f; // 75.3% mono tint, 75.3% alpha colortag
        render_color[1] = 0.753f;
        render_color[2] = 0.753f;
        render_color[3] = 0.753f;
