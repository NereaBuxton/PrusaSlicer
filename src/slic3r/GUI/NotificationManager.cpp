...
{
	if (m_is_gray) {
		ImVec4 backcolor(0.753f, 0.753f, 0.753f, 0.502f); // 75.3% mono tint, 50% alpha
...
void NotificationManager::PopNotification::render_hypertext(ImGuiWrapper& imgui, const float text_x, const float text_y, const std::string text, bool more)
{
	...
	//hover color
	ImVec4 orange_color = ImVec4(0.910f, 0.529f, 0.102f, 1.0f); // categorical 3 colortag
	if (ImGui::IsItemHovered(ImGuiHoveredFlags_RectOnly)) { // categorical 10 colortag
		orange_color.x = 0.796f;
		orange_color.y = 0.435f;
		orange_color.z = 0.098f;
	}
...
void NotificationManager::PopNotification::render_close_button(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
	...
	ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
	push_style_color(ImGuiCol_Text, ImVec4(.980f, .980f, .980f, 1.f), m_state == EState::FadingOut, m_current_fade_opacity); // 98% near-white colortag
	push_style_color(ImGuiCol_TextSelectedBg, ImVec4(.098f, .753f, .780f, 1.f), m_state == EState::FadingOut, m_current_fade_opacity); // categorical 1 colortag
	ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(.0f, .0f, .0f, .0f));
...
void NotificationManager::PopNotification::render_minimize_button(ImGuiWrapper& imgui, const float win_pos_x, const float win_pos_y)
{
	...
	push_style_color(ImGuiCol_Text, ImVec4(.980f, .980f, .980f, 1.f), m_state == EState::FadingOut, m_current_fade_opacity); // 98% near-white colortag
	push_style_color(ImGuiCol_TextSelectedBg, ImVec4(.098f, .753f, .780f, 1.f), m_state == EState::FadingOut, m_current_fade_opacity); // ccategorical 1 colortag
...
{
	ImVec4 orange_color			= ImVec4(0.910f, 0.529f, 0.102f, 1.0f); // categorical 3 colortag
	ImVec4 gray_color			= ImVec4(0.251f, 0.251f, 0.251f, 1.0f); // 25.1% mono shade colortag
