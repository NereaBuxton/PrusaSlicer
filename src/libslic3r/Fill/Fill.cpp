...
//            printf("flow spacing: %f,  is_external: %d, link_max_length: %lf\n", flow.spacing(), int(surface.is_external()), link_max_length);
		...
		f->loop_clipping = coord_t(scale_(surface_fill.params.flow.nozzle_diameter()) * object()->config().loop_clipping_length * 0.01);
		...
		auto w = surface_fill.params.flow.scaled_width();

        for (ExPolygon &expoly : with_sheath ? closing_ex(to_polygons(surface_fill.expolygons), float(SCALED_EPSILON), float(SCALED_EPSILON) + w) : surface_fill.expolygons) {
		...
		if (with_sheath) {
			for (ExPolygon& expoly : closing_ex(to_polygons(surface_fill.expolygons), float(SCALED_EPSILON), float(SCALED_EPSILON) + 0.5 * w)) {
				Polylines polylines;
