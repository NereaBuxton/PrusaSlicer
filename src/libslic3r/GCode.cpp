...
//            config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
    ...
    const std::vector<std::string> ColorPrintColors::Colors = { "#C0392B", "#E67E22", "#F1C40F", "#27AE60", "#1ABC9C", "#2980B9", "#9B59B6" }; // rainbow colors colours: red, orange, yellow, green, blue, blue (is that actually indigo?), purple
    ...
    for (unsigned int extruder_id : layer_tools.extruders)
    {
        ...
        for (int print_wipe_extrusions = is_anything_overridden; print_wipe_extrusions>=0; --print_wipe_extrusions) {
            ...
            for (InstanceToPrint &instance_to_print : instances_to_print) {
                ...
                if (!m_config.optimize_island_extrusions) {
...
//    description += ExtrusionEntity::role_to_string(path->role);
        path->simplify(
            ...
            std::min(std::max(scale_(0.02), m_scaled_resolution), 0.25 * scale_(EXTRUDER_CONFIG(nozzle_diameter))));
    ...
    if (m_config.little_move_inwards && paths.back().role() == erExternalPerimeter && m_layer != NULL && m_config.perimeters.value > 1 && paths.front().size() >= 2 && paths.back().polyline.points.size() >= 3) {
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed)
{
    ...
    if (m_config.default_acceleration.value > 0) {
        ...
        } else if (m_config.solid_infill_acceleration.value > 0 && (path.role() == erSolidInfill || path.role() == erTopSolidInfill)) {
        ...
        } else if (m_config.support_material_acceleration.value > 0 && path.role() == erSupportMaterial) {
            acceleration = m_config.support_material_acceleration.value;
        } else if (m_config.support_material_interface_acceleration.value > 0 && path.role() == erSupportMaterialInterface) {
            acceleration = m_config.support_material_interface_acceleration.value;
        } else {
...
bool GCode::needs_retraction(const Polyline &travel, ExtrusionRole role)
{
    ...
    if (role == erSupportMaterial && m_config.retract_skip_support_islands) {
