    ...
    m_slicing_params        (slicing_params)
{
    ...
    m_support_params.with_interface_sheath  = (m_object_config->support_material_interface_with_sheath && m_object_config->support_material_interface_layers.value > 0) || (m_object_config->support_material_interface_pattern == smipAuto && m_slicing_params.soluble_interface && m_object_config->support_material_interface_layers.value > 0);
    ...
    m_support_params.contact_fill_pattern =
        ...
        m_object_config->support_material_interface_pattern == smipRectilinear ?
        (m_support_params.interface_density > 0.95 || m_support_params.with_interface_sheath ? ipRectilinear : ipSupportBase) :
        ipConcentric;        
}
    ...
    const std::vector<Polygons> &layer_support_areas) const
{
    ...
    tbb::parallel_for(
        ...
        filter, filter_disabled, filter_or_expansion_width, support_width, gap](const tbb::blocked_range<size_t>& range) {
...
//    my $area_threshold = $self->interface_flow->scaled_spacing ** 2;
    ...
    if (! intermediate_layers.empty() && (num_interface_layers_top > 1 || num_interface_layers_bottom > 1)) {
        ...
        auto minimum_island_radius =
            m_support_params.with_interface_sheath ? smoothing_distance * 5.5 + scaled(m_support_params.gap_xy) :
            smoothing_distance * 2.0 + scaled(m_support_params.gap_xy);
        ...
        if (role == erSupportMaterialInterface) {
            for (ExPolygon& expoly2 : closing_ex(expoly, float(SCALED_EPSILON), float(SCALED_EPSILON + 1.0 * flow.scaled_width()))) {
                ...
                for (size_t i = 0; i <= expoly2.holes.size(); ++i) {
                    ...
                    pl2.clip_end(clip_length);
                    polylines.emplace_back(std::move(pl2));
                }
            }

            extrusion_entities_append_paths(out, polylines, role, flow.mm3_per_mm(), flow.width(), flow.height());
            // Fill in the rest.
...
//    const coordf_t link_max_length_factor = 3.;
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        ...
        auto filler_base_interface  = std::unique_ptr<Fill>(base_interface_layers.empty() ? nullptr : 
            Fill::new_from_type(m_support_params.interface_density > 0.95 || m_support_params.with_sheath || m_support_params.with_interface_sheath ? ipRectilinear : ipSupportBase));
